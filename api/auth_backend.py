#!/usr/bin/env python3
"""
Stateless Authentication Backend for Companion AI
...
class AuthManager:
    ...
    def _get_or_create_secret(self) -> str:
        ...
        # For development, use a fixed secret to ensure consistency
        return "companion-ai-jwt-secret-key-2025-fixed-for-debugging"
    ...
    def clear_all_sessions(self):
        """Clear all active sessions (for development/testing)"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def _generate_jwt_token(self, user_id: int, email: str, name: str, remember: bool = False) -> str:
        ...
        expiry_hours = 30 * 24 if remember else 7 * 24  # 30 days if remember, 7 days otherwise
    ...
    def login_user(self, email: str, password: str, ip_address: str = "", user_agent: str = "", remember: bool = False) -> Tuple[bool, str, Optional[Dict]]:
        ...
        # Check rate limiting
        if self._check_rate_limit(email, ip_address):
        ...
        try:
            ...
            # Generate JWT token
            token = self._generate_jwt_token(user_id, user_email, name, remember)

            # Store session
            token_hash = hashlib.sha256(token.encode()).hexdigest()
            expires_at = datetime.utcnow() + timedelta(days=30 if remember else 7)
